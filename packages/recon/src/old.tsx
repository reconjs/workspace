...
const Generator = NON_ITERATOR().constructor
const AsyncGenerator = NON_ASYNC_ITERATOR().constructor
    ...
    const render = dispatcher (() => {
      for (let i = 0; i < MAX; i++) {
        ...
        if (value instanceof Subscription) {
          ...
          console.warn ("Unknown Effect!!!!!!", value)
        }
      }
...
const entrypoints = {} as Record<string, {
  scope: ReconScope,
  proc: Proc,
}>

WINDOW.__RECON_ENTRYPOINTS__ = entrypoints
...
export function use$ (resource: any, ...params: any[]): never {
  ...
  const { proc, scope } = (entrypoints[id] ??= {
    proc: resource,
    scope: new ReconScope (parent),
  })
